...
evalLiteral = do
    (env, LiteralExpr val) <- next    --call next, bind its output to a tuple (env, LiteralExpr val)
...
-- evaluates a comparison, specifically equal? and <
...
evalNotExpr = do
    ...
    case getValue $ eval evalExpr (env, expr) of 
        Right (BoolVal b) -> return (BoolVal (not b))
        Left err -> evalError err
        ...
        Right (ClosureVal "" argName funBody cenv) ->
            let env' = Env.bind letName (ClosureVal letName argName funBody cenv) cenv in
                case getValue (eval evalExpr (env', body)) of
                    ...
                    Left err -> evalError err
        Right nameval ->
            case getValue (eval evalExpr (Env.bind letName nameval env, body)) of
...
evalLambdaExpr = do  
    (env, LambdaExpr arg code) <- next
    return (ClosureVal "" arg code env) 
...
callFun (ClosureVal func argStr code cenv) argVal = do
    let newEnv = (Env.bind argStr argVal cenv)
    case getValue $ eval evalExpr (newEnv, code) of
        Right val -> return val
        Left err -> Left err
callFun _ _ = error "callFun needs a closure"
...
evalApplyExpr = do
    (env, ApplyExpr expr1 expr2) <- next
    case eval evalExpr (env, expr1) of
        Right(c@(ClosureVal funcName argName code cenv), _) ->
            case eval evalExpr (env, expr1) of
                Right (val, _) ->
                    case callFun c val of 
                        Right v -> return v
                        Left err -> evalError err
                Left err -> evalError err
        _ -> evalError $ EvalError "in evalApplyExpr, must be a closure."
